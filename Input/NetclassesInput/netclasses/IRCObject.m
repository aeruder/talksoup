...
static void rec_cdcc(IRCObject *client, NSString *prefix,
                       NSString *command, NSString *rest, NSString *to)
{
	NSArray *list = [rest componentsSeparatedByString: @" "];
	
	if ([list count] < 4)
	{
		return;
	}
	if ([[list objectAtIndex: 0] isEqualToString: @"SEND"])
	{
		id fileName;
		id fileSize;
		id port;
...
@implementation IRCObject (DCCSupport)
...
- (NSData *)DCCNeedsMoreData: aConnection
{
	return nil;
}
- sendDCCSendRequest: (NSDictionary *)info to: (NSString *)person
{	
	NSString *file = [info objectForKey: DCCInfoFileName];
	NSNumber *size = [info objectForKey: DCCInfoFileSize];
	NSNumber *port = [info objectForKey: DCCInfoPort];
	NSString *address = [info objectForKey: DCCInfoHost];
	struct in_addr inp;
	
	if (contains_a_space(file) || contains_a_space(person))
	{
		[NSException raise: IRCException format:
		 @"[IRCObject sendDCCRequest: '%@' to: '%@'] No spaces allowed",
		  info, person];
	}

	inet_aton([address cString], &inp);
	
	[self writeString: @"PRIVMSG %@ :\001DCC SEND %@ %lu %hu %lu\001",
	 person, file, ntohl(inp.s_addr), [port unsignedShortValue], 
...
- initWithDelegate: (IRCObject *)aDelegate withInfo: (NSDictionary *)aInfo
...
- dataReceived: (NSData *)data
{
	...
	[transport writeData: [NSData dataWithBytes: &len length: 4]];
...
- (void)connectionLost
{
	if (status == DCCStatusTransferring)
	{
...
- initWithSendOfFile: (NSString *)name
    ...
    withDelegate: (IRCObject *)aDelegate
    withTimeout: (int)seconds withBlockSize: (int)numBytes
	withUserInfo: (NSDictionary *)aUserInfo
{
	id address;
	id portNum;
